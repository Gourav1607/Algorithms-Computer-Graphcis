...
void input()
{
	...
	/*
	 for(i=0;i<=n;i++)
	 {
	 a[i][0]=a[i][0]+getmaxx()/2;
	 a[i][1]=getmaxy()/2-a[i][1];
	 }
	 */
}
...
void output()
{
	...
	for (i = 0; i < n; i++)
	{
		line(a[i][0] + getmaxx() / 2, getmaxy() / 2 - a[i][1],
			 a[i + 1][0] + getmaxx() / 2, getmaxy() / 2 - a[i + 1][1]);
	}
}
...
void reflection()
{
	...
	case 4:
		for (i = 0; i <= n; i++)
		{ // multiply by -1 and then swap x y
			int swap;
			a[i][0] = (-1) * a[i][0];
			a[i][1] = (-1) * a[i][1];
			swap = a[i][0];
	...
	do
	{
		...
		if (j == 1)
		{
			...
			for (i = 0; i < n; i++)
			{
				...
				printf("%d %d %d %d", a[i][0], a[i][1], a[i + 1][0],
					   a[i + 1][1]);
			}
		}
	} while (j == 1);
